...
export function LearningResources({ assignment, course, searchQuery }: LearningResourcesProps) {
  ...
  useEffect(() => {
    ...
    async function fetchRecommendations() {
      ...
      try {
        ...
        const resp = await ApiService.getAssignmentRecommendations(assignment.id, params)
        if (resp.error) {
        ...
        if (meta?.sources) {
          console.log('[recommendations] source counts:', meta.sources)
        } else {
          console.log('[recommendations] no meta.sources, raw payload:', resp.data)
        }
        const data: RecResponse = resp.data as any
        if (aborted) return
        setBackendQuery(data.query)
        const mapped: LearningResource[] = (data.results || []).map((r) => {
          const type: LearningResource["type"] = r.source === "youtube" ? "youtube" : "website"
          return {
            title: r.title,
            url: r.url,
            type,
  ...
  }, [assignment?.id, submittedQuery])

  // 後端結果優先，後備使用本地規則
  const defaultResources = useMemo(() => {
    if (backendResources.length > 0) return backendResources
    return getResourcesByKeywords(keywords)
  }, [backendResources, keywords])
