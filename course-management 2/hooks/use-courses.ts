...
export function useCourses(lineUserId: string) {
  ...
  const deleteCourse = useCallback(async (id: string) => {
    try {
      ...
      // 極速回饋：先行從前端移除（樂觀更新），失敗再回滾
      let rollbackCourses: typeof courses | null = null
      let rollbackAssignments: typeof assignments | null = null
      let rollbackNotes: typeof notes | null = null
      let rollbackExams: typeof exams | null = null

      setCourses(prev => {
        rollbackCourses = prev
        return prev.filter(course => course.id !== id)
      })
      setAssignments(prev => {
        rollbackAssignments = prev
        return prev.filter(assignment => assignment.courseId !== id)
      })
      setNotes(prev => {
        rollbackNotes = prev
        return prev.filter(note => note.courseId !== id)
      })
      setExams(prev => {
        rollbackExams = prev
        return prev.filter(exam => exam.courseId !== id)
      })

      const response = await ApiService.deleteCourse(courseId)
      if (response.error) {
        // 回滾
        if (rollbackCourses) setCourses(rollbackCourses)
        if (rollbackAssignments) setAssignments(rollbackAssignments)
        if (rollbackNotes) setNotes(rollbackNotes)
        if (rollbackExams) setExams(rollbackExams)
        throw new Error(response.error)
      }
    } catch (err) {
      throw err
    ...
    try {
      ...
      if (response.data) {
        ...
        const wantIds = Array.isArray((note as any).attachments)
          ...
          : []
        if (wantIds.length > 0 && (!response.data.attachments || response.data.attachments.length === 0)) {
