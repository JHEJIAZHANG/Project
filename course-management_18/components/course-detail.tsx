...
export function CourseDetail({ courseId, showBackButton = true, onViewAssignment, onViewExam }: CourseDetailProps) {
  ...
  const getAssignmentDateColor = (status: Assignment["status"]) => {
    switch (status) {
      ...
      default:
        return "text-amber-700" // Amber for in progress
    }
  }
  ...
  const getExamDateColor = (exam: Exam) => {
    ...
    if (exam.status === "completed" || daysDiff < 0) {
      ...
      return "text-amber-700" // Amber for upcoming (within 7 days)
    } else {
