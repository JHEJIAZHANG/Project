...
struct TaskView: View {
    ...
    @State private var todos: [TodoItem] = [
        TodoItem(title: "資料庫系統作業", category: .study, dueDate: Calendar.current.date(bySettingHour: 23, minute: 59, second: 0, of: Date()), priority: .high, estimatedDuration: 120),
        TodoItem(title: "購買計算機概論教科書", category: .life, dueDate: Calendar.current.date(byAdding: .day, value: 2, to: Date()), priority: .medium, estimatedDuration: 30),
        TodoItem(title: "網頁程式設計小組討論", category: .study, dueDate: Calendar.current.date(byAdding: .day, value: 3, to: Date())?.addingTimeInterval(15*3600+30*60), priority: .low, estimatedDuration: 90),
        TodoItem(title: "繳交社團費用", category: .life, dueDate: Calendar.current.date(byAdding: .day, value: 5, to: Date()), priority: .medium, estimatedDuration: 10),
        TodoItem(title: "行動應用開發報告", category: .study, dueDate: Calendar.current.date(byAdding: .day, value: -1, to: Date()), priority: .high, estimatedDuration: 180, isCompleted: true) // Corrected order
    ]
...
struct TodoRowView: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            ...
            // Priority Tag
            Text(todo.priority.rawValue) // Use rawValue to display
                .font(.caption)
...
struct AddTaskView: View {
                 ...
                 Section("任務資訊") {
                     ...
                     Picker("優先級", selection: $selectedPriority) {
                         ForEach(TodoItem.Priority.allCases) { prio in
                             Text(prio.rawValue).tag(prio) // Use rawValue from Core/Models/TodoItem.swift
                         }
                     }
                 }
                 ...
                 Section("時間與提醒") {
                     ...
                     Picker("提醒", selection: $selectedReminder) {
                         ForEach(TodoItem.ReminderOption.allCases, id: \.rawValue) { reminder in
                             Text(reminder.rawValue).tag(reminder) // Use rawValue from Core/Models/TodoItem.swift
                         }
                     }
                 }
    ...
    private func addTask() {
        ...
        let newTodo = TodoItem(
            ...
            estimatedDuration: nil // Explicitly add nil for estimatedDuration
        )
