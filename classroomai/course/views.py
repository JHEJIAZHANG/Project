...
from .models import Course, Homework, CourseSchedule, StudentNote, FileAttachment
...
def create_file_upload(request):
    ...
    try:
        ...
        # 驗證必要參數
        if not line_user_id or not file_name or not file_url:
            return Response({
                "error": "缺少必要參數",
                "message": "請提供 line_user_id, file_name, file_url 參數"
            }, status=400)
        
        # 獲取用戶
        try:
            author = LineProfile.objects.get(line_user_id=line_user_id)
        except LineProfile.DoesNotExist:
            return Response({"error": "用戶不存在"}, status=404)
        ...
        if note_id:
            try:
                note = StudentNote.objects.get(id=note_id, author=author)
            except StudentNote.DoesNotExist:
                return Response({"error": "指定的筆記不存在或無權限訪問"}, status=400)
