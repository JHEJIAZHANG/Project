...
def _update_custom_todo(request, todo_id):
    ...
    try:
        ...
        if due_date_field:
            is_valid, due_date, error_response = APIValidator.validate_datetime_field(
                data, due_date_field, required=False
            )
            if not is_valid:
                return error_response
            if due_date:
...
def get_notes(request):
    ...
    HEAD /api/v2/notes/?line_user_id=U123456789
    POST /api/v2/notes/ (with line_user_id in body)
    """
    if request.method in ["GET", "HEAD"]:
...
def _get_notes_list(request):
    ...
    try:
        ...
        return APIValidator.create_success_response(formatted_notes)
...
def get_exams(request):
    ...
    try:
        ...
        # 獲取用戶的考試
        from api_v2.models import ExamV2
        exams = ExamV2.objects.filter(user=user).select_related('course').order_by('exam_date')
        ...
        logger.info(f"Retrieved {len(formatted_exams)} exams for user: {user.line_user_id}")
        
        return APIValidator.create_success_response(formatted_exams)
    
    except Exception as e:
        logger.error(f"Error retrieving exams: {str(e)}", exc_info=True)
        return Response({
            "error": "internal_error",
            "message": "獲取考試時發生錯誤",
            "details": str(e),
