...
class CourseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'description', 'instructor', 'classroom', 'color', 
                  'is_google_classroom', 'google_classroom_id', 'google_classroom_url', 'schedules', 
...
class AssignmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'course', 'title', 'description', 'due_date', 'notification_time',
                  'type', 'status', 'google_coursework_id', 'google_classroom_url', 'attachments', 'created_at', 'updated_at']
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class LineProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LineProfile
        fields = [
            'line_user_id', 'name', 'email', 'role',
            'preferences', 'settings', 'last_active', 'extra'
        ]
        read_only_fields = ['line_user_id', 'role']
    
    def get_preferences(self, obj):
        """從 extra 欄位獲取偏好設定"""
        return obj.extra.get('preferences', {})
    
    def get_settings(self, obj):
        """從 extra 欄位獲取設定"""
        return obj.extra.get('settings', {})
    
    def get_last_active(self, obj):
        """從 extra 欄位獲取最後活躍時間"""
        last_active = obj.extra.get('last_active')
        return last_active
    
    def update(self, instance, validated_data):
        """更新用戶配置文件"""
        # 處理 preferences 和 settings
        preferences = self.initial_data.get('preferences')
        ...
        if preferences is not None:
            ...
            if isinstance(preferences, dict):
                ...
                instance.extra['preferences'] = filtered_preferences
        
        if settings is not None:
            if isinstance(settings, dict):
                instance.extra['settings'] = settings
        
        # 更新最後活躍時間
        from django.utils import timezone
        instance.extra['last_active'] = timezone.now().isoformat()
        
        # 更新其他欄位
        for attr, value in validated_data.items():
            if attr not in ['preferences', 'settings', 'last_active']:
                setattr(instance, attr, value)
